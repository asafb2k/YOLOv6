...
if __name__ == "__main__":
	weights=Path(r"R:\Asaf\best_models\scoreboard_detection\models for testing\exp8, first tiny experiment heavy augmentations 416x416 100 last epochs without heavy augmentations\weights\best_stop_aug_ckpt.pt").as_posix()
	# weights=Path(r"R:\Asaf\best_models\scoreboard_detection\Yolov6_SMALL_with_heavy_augmentations_14_11_2022\exp2 (with heavy augmentation)\exp2 (with heavy augmentation)\weights\best_stop_aug_ckpt.pt").as_posix()
	yaml = Path(r"A:\YOLOv6\data\dataset.yaml").as_posix()
	img_size = (416,416,3)
	detector = Yolov6Detector(weights,yaml,img_size=img_size[0])
	...
	iou_threshold = 0.95
	mlflow_output_path = Path(r"R:\model mangement\mlruns\1")
	...
	# evaluate(detector,bucket,model,date,sport, iou_threshold,img_size,mlflow_output_path)
